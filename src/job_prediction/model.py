...
class SubmissionFeatureEncoder:
    ...
    def fit(cls, frame: pd.DataFrame) -> SubmissionFeatureEncoder:
        _require_columns(frame, SUBMISSION_FEATURES)
...
class JobPredictor:
    ...
    def load(cls, path: str | Path) -> JobPredictor:
        ...
        if values.get("format_version") != _MODEL_FORMAT_VERSION:
...
def _completed_by(
    ...
    *,
    ...
    _require_columns(frame, (COMPLETION_TIME,))
    completion_times = pd.to_datetime(
...
def fit_gradient_predictor(
    ...
    *,
    ...
    def rows_of(frame: pd.DataFrame, cutoff: object, period: str) -> np.ndarray:
        available = _completed_by(frame, cutoff, period_name=period)
